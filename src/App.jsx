  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .car-img-card:hover {
      ...
      box-shadow: 0 20px 48px rgba(26,24,20,0.18);
    }
    .car-img-card img {
      width: 100%;
      height: 100%;
...
function StepIcon({ name, color }) {
  ...
  const icons = {
    clipboard: (<svg width="20" height="20" viewBox="0 0 24 24" style={s}><rect x="9" y="2" width="6" height="4" rx="1"/><path d="M9 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2h-3"/><path d="M9 12h6M9 16h4"/></svg>),
    broadcast: (<svg width="20" height="20" viewBox="0 0 24 24" style={s}><path d="M4.93 4.93a10 10 0 0 0 0 14.14"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M7.76 7.76a6 6 0 0 0 0 8.48"/><path d="M16.24 7.76a6 6 0 0 1 0 8.48"/><circle cx="12" cy="12" r="1.5"/></svg>),
    timer: (<svg width="20" height="20" viewBox="0 0 24 24" style={s}><circle cx="12" cy="13" r="8"/><path d="M12 9v4l3 2"/><path d="M9 2h6M12 2v3"/></svg>),
    checkCircle: (<svg width="20" height="20" viewBox="0 0 24 24" style={s}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="M22 4 12 14.01l-3-3"/></svg>),
  };
...
function PerkIcon({ name }) {
  ...
  const icons = {
    target: (<svg width="22" height="22" viewBox="0 0 24 24" style={s}><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>),
    barChart: (<svg width="22" height="22" viewBox="0 0 24 24" style={s}><path d="M18 20V10M12 20V4M6 20v-6"/></svg>),
    zap: (<svg width="22" height="22" viewBox="0 0 24 24" style={s}><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/></svg>),
    trendingUp: (<svg width="22" height="22" viewBox="0 0 24 24" style={s}><path d="m23 6-9.5 9.5-5-5L1 18"/><path d="M17 6h6v6"/></svg>),
    users: (<svg width="22" height="22" viewBox="0 0 24 24" style={s}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>),
    smartphone: (<svg width="22" height="22" viewBox="0 0 24 24" style={s}><rect x="5" y="2" width="14" height="20" rx="2"/><path d="M12 18h.01"/></svg>),
  };
  return icons[name] || null;
}
...
function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({ length: 140 }, () => ({
      ...
      r: Math.random() * 1.6 + 0.2, baseOpacity: Math.random() * 0.35 + 0.05,
      ...
      <div className={`lg fade-up-1 ${scrolled ? "nav-scrolled" : ""}`}
        ...
        <div style={{ flex:1 }}>
          <a href="/" style={{ textDecoration:"none" }}>
            <span className="serif-italic" style={{ fontSize:"1.25rem", fontWeight:400, letterSpacing:"-0.02em", color:"#1a1814", cursor:"pointer" }}>
...
function CarMarquee() {
  ...
  const doubled = [...MARQUEE_CARS, ...MARQUEE_CARS];
  return (
...
function AuctionDemo() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      const newAmt = bids[0].amount + Math.floor(Math.random()*250+100);
      const colors = ["var(--accent)","var(--accent2)","var(--accent3)"];
      setBids(prev => [{ dealer:pick, amount:newAmt, time:fmtTime(), color:colors[Math.floor(Math.random()*3)] }, ...prev].slice(0,5));
      setHighlight(0);
    ...
    <div className="lg" style={{ borderRadius:"1.75rem", padding:"2.25rem", width:"100%", border:"1px solid rgba(26,24,20,0.08)" }}>
      <div style={{ display:"flex", gap:"1rem", marginBottom:"1.25rem", alignItems:"center" }}>
        ...
        <div>
          ...
          <div style={{ color:"rgba(26,24,20,0.82)", fontSize:"0.85rem", marginTop:"0.2rem" }}>94,200 mi · Chicago, IL</div>
        </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:"0.15rem" }}>
        {bids.map((b,i) => (
          <div key={b.dealer+b.time+i} className="bid-row" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0.5rem 0.6rem", borderRadius:"0.6rem", background: i === highlight ? "rgba(124,92,191,0.07)" : "transparent", animation: i === highlight ? "bidPop 0.5s ease" : "none", transition:"background 0.4s" }}>
...
const steps = [
  ...
  { n:"03", title:"Open live auction", body:"Watch real bids roll in on your phone. Dealers compete knowing only one can win — pushing offers higher than they'd go one-on-one.", icon:"timer", color:"var(--accent3)" },
  { n:"04", title:"Accept & hand off", body:"Accept the best offer whenever you're ready. The winning dealer arranges pickup. You get paid.", icon:"checkCircle", color:"var(--accent)" },
];
...
function ForDealers() {
  const perks = [
    ...
    { icon:"trendingUp", title:"High-margin stock", body:"6–8 year old vehicles with 80k–120k miles are the bread-and-butter of pre-owned profits." },
    { icon:"users", title:"Exclusive 5-dealer auctions", body:"You're never bidding against 40 anonymous buyers. Five dealers, clean competition." },
    ...
    <section id="for-dealers" style={{ background:"#edeae4", padding:"7rem 1.5rem" }}>
      <div style={{ maxWidth:"64rem", margin:"0 auto" }}>
        ...
        <div className="reveal" style={{ borderRadius:"1.5rem", overflow:"hidden", height:280, marginBottom:"4rem", position:"relative", border:"1px solid rgba(26,24,20,0.10)", boxShadow:"0 12px 40px rgba(26,24,20,0.14)" }}>
          <img src="https://images.unsplash.com/photo-1770319942638-a5989632f2ad?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" alt="Dealer lot" alt="Dealer lot" alt="Dealer lot" alt="Dealer lot" alt="Dealer lot" style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center 40%" }} loading="lazy" />
...
function About() {
  return (
    <section id="about" style={{ background:"#f5f3ef", padding:"7rem 1.5rem" }}>
      <div style={{ maxWidth:"64rem", margin:"0 auto" }}>
        <div style={{ display:"flex", gap:"4rem", flexWrap:"wrap", alignItems:"center" }}>
          ...
          <div style={{ flex:"1 1 280px" }} className="reveal">
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.75rem", height:320 }}>
              <div className="car-img-card" style={{ gridRow:"1 / 3" }}>
                <img src="https://images.unsplash.com/photo-1567199935819-e2fcd8d35ecd?w=400&q=80" alt="Car lot" loading="lazy" />
              </div>
              <div className="car-img-card">
                <img src="https://images.unsplash.com/photo-1590361936226-f85a300ff1a7?w=300&q=80" alt="Cars parked" loading="lazy" />
              </div>
              <div className="car-img-card">
                <img src="https://images.unsplash.com/photo-1471479917193-f00955256257?w=300&q=80" alt="Dealer lot" loading="lazy" />
              </div>
            </div>
          </div>
          ...
          {/* Text side */}
          <div style={{ flex:"1 1 320px" }} className="reveal">
            <span className="section-badge">About PrivaBuy</span>
            <h2 className="serif-italic" style={{ fontSize:"clamp(2rem,4vw,3rem)", lineHeight:0.93, letterSpacing:"-0.04em", margin:"0 0 1.25rem", fontWeight:400 }}>
              Built for the gap<br />the market ignores.
            </h2>
            <p style={{ color:"rgba(26,24,20,0.84)", fontSize:"0.95rem", lineHeight:1.75, marginBottom:"1.25rem" }}>
              PrivaBuy was founded by automotive and tech entrepreneurs who kept seeing the same problem: private sellers were leaving thousands of dollars on the table, and franchised dealers were struggling to source the exact inventory their customers wanted.
            </p>
            <p style={{ color:"rgba(26,24,20,0.84)", fontSize:"0.95rem", lineHeight:1.75, marginBottom:"1.75rem" }}>
              The 6–8 year old, higher-mileage vehicle is the most profitable segment in pre-owned automotive retail — and the hardest to source at scale. We built a two-sided marketplace that creates genuine competition for every listing, turning what used to be a one-sided negotiation into a real auction.
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
              {[
                { label:"Mission", value:"Make vehicle transactions fair for sellers by creating real competition." },
                { label:"Coverage", value:"Currently serving Chicago and expanding to major metros in 2026." },
                { label:"Dealer Network", value:"Verified, licensed franchised dealers only. No wholesalers, no flippers." },
              ].map(({ label, value }) => (
                <div key={label} style={{ display:"flex", gap:"1rem", padding:"0.875rem 1rem", background:"rgba(255,255,255,0.60)", borderRadius:"0.75rem", border:"1px solid rgba(26,24,20,0.10)" }}>
                  <div style={{ fontSize:"0.72rem", fontWeight:600, color:"var(--accent)", textTransform:"uppercase", letterSpacing:"0.06em", flexShrink:0, paddingTop:"0.1rem", minWidth:80 }}>{label}</div>
                  <div style={{ fontSize:"0.85rem", color:"rgba(26,24,20,0.84)", lineHeight:1.5 }}>{value}</div>
                </div>
              ))}
            </div>
          </div>
...
function FAQ() {
  ...
  return (
    <section id="faq" style={{ background:"#edeae4", padding:"7rem 1.5rem" }}>
...
function CTA() {
  return (
    <section style={{ background:"#f5f3ef", padding:"7rem 1.5rem 8rem", position:"relative", overflow:"hidden" }}>
      {/* Background car image */}
      <div style={{ position:"absolute", inset:0, zIndex:0 }}>
        <img src="https://images.unsplash.com/photo-1485291571150-772bcfc10da5?w=1600&q=80" alt="" loading="lazy" style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center", opacity:0.08 }} />
      </div>
